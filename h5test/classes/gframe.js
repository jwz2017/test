...
const STATE_WAIT_FOR_CLOSE = "statewaitforclose",
  ...
  STATE_HIGH_SCORES = "statehighscores";
...
function _waitComplete() {
  ...
  if (!game.enableMouseOver) stage.enableMouseOver(0);
...
function _systemGamePlay(e) {
  ...
  if (game.levelUp) {
    ...
    if (game.level >= game.maxLevel) {
      if(!game.enableMouseOver)stage.enableMouseOver();
    ...
    if (game.highScoreScreen) {
      _nextSystemState = STATE_HIGH_SCORES;
    } else _nextSystemState = STATE_GAME_OVER;
    if(!game.enableMouseOver)stage.enableMouseOver();
    _switchSystemState(STATE_WAIT_FOR_CLOSE);
  }
};
...
function _highScore() {
  stage.addChild(game.highScoreScreen);
  _switchSystemState(STATE_WAIT_FOR_CLOSE);
...
function buildGame(GClass, isDamo) {
  ...
  window.onblur = _systemPause;
  window.onfocus = _systemPause;
