 ...
 * @param {*} actor 
 * @param {*} velocity 
 * @param {*} pressed 最后一个按键
 */
export function floorMove(actor,velocity,pressed){
    let s=actor.speed;
    s.x=0;
    if(pressed=="left") s.x=-velocity;
    else if(pressed=="right") s.x=velocity;
}
 ...
 * @param {*} pressed 最后一个按键
 */
export function tankMove(actor,velocity, pressed) {
    actor.speed.zero();
    if (pressed) actor.image.paused = false;
