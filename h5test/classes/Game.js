...
class Game extends ScrollContainer {
    ...
    static createrContainer = function (parent) {
        let c = new createjs.Container();
        c.name = "gameLayer";
        if (parent) parent.addChild(c);
        return c;
    };
    static clearContainer = function (container) {
        ...
        for (let i = l; i >= 0; i--) {
            ...
            if (element.name == "gameLayer") {
                ...
                if (element.recyle) element.recyle();
                else container.removeChild(element);
            }
        };
    };
    ...
    constructor(titleText, width = stage.width, height = stage.height) {
        ...
        //游戏层
        this.floorLayer = Game.createrContainer(this.container);
        this.moveLayer = Game.createrContainer(this.container);
        this.playerLayer = Game.createrContainer(this.container);
        //网格
        this._tempNode = new Node(-1, -1);
        this.playerChars = Object.create(null);
        this.noMoveChars = Object.create(null);
        this.moveChars = Object.create(null);
        //游戏属性
        this.gameOver = false;
        this.levelUp = false;
        this.maxLevel = 1;
    ...
    createLevelInScreen() {
        ...
        this.levelInScreen.createTextElement(gframe.LevelInLevel, 1, 0, 0, { font:"bold 40px regul,Arial"})
    ...
    createPauseScreen() {
        ...
        let text = this.pauseScreen.createText("pause","bold 60px regul,Arial");
        let b = text.getBounds();
        text.x = this.width - b.width >> 1;
        text.y = this.height - b.height >> 1;
    }
    createHighScoreScreen(){
        // this.highScoreScreen=new TitleScreen("highscore",this.width,this.height);
        // let btn1 = this.highScoreScreen.createButton("关闭");
        // btn1.x = stage.width - btn1.getBounds().width >> 1;
        // btn1.y = stage.height * 0.6;
    }
    /***************************游戏开始状态************************ */
    createScoreBoard() { }
    newGame() { }
     ...
     */
    hasParent(array) {
        return array.some(function (actor) {
            return actor.parent;
        })
    }
    ...
    createGrid(step, Actor, cols, rows) {
        ...
        for (let i = 0; i < col; i++) {
            ...
            for (let j = 0; j < row; j++) {
                ...
                if (Actor) {
                    node.actor = Actor.getActor();
                    node.actor.init(i * step, j * step)
    ...
    createGridMap(plan, stepWidth, stepHeight, drawGrid, isIso = false) {
        ...
        for (let x = 0; x < numCols; x++) {
            ...
            for (let y = 0; y < numRows; y++) {
                ...
                const ch = plan[y][x],
                    Act = this.playerChars[ch]||this.noMoveChars[ch] || this.moveChars[ch];
                if (Act) {
                    let a = Actor.getActor(Act);
                    a.init(x * this.stepWidth, y * this.stepHeight, ch);
                    switch (Act) {
                        case this.playerChars[ch]:
                            this.addToPlayer(a);
                            if (a.name == "player") this.player = a;
                            break;
                        case this.noMoveChars[ch]:
                            this.container.addChild(a);
                            node.type = Node.PROP;
                            node.actor = a;
                            break;
                        case this.moveChars[ch]:
                            this.addToMove(a);
                            break;
                    }
                }
                if (drawGrid) drawGrid(ch, node, Act);
            }
        }
        if (isIso) {
            //等角地图深度排序
    ...
    */
    ...
    get moveChildren() {
        return this.moveLayer.children;
    }
    get floorChildren() {
        return this.floorLayer.children;
    }
    get playerChildren() {
    ...
    //分数排行榜
    getHighScores(){
        let key=this.titleText+"_highScore",
     ...
     * 设置排行榜
     * @param {*} highScore 
     * @param {*} highScores getHighScores()
     */
...
class Box2dGame extends ScrollMapGame {
    ...
    constructor(titleText, isDebug = true, gravity = 10, width, height) {
        ...
        this.box2DLayer = Game.createrContainer(this.container)
    }
