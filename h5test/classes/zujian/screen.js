...
class BasicScreen extends createjs.Container {
     ...
     * 创建dom文本
     * @param {string} text 
     * @param {string} font = "30px regul,Arial,Microsoft Yahei"
     * @param {string} color= "#fff"
     * @returns 
     */
    createText(text, font="30px regul,Arial,Microsoft Yahei", color = "#fff") {
        ...
        txt.setBounds(0, 0, ele.clientWidth, ele.clientHeight);
        return txt;
    }
}
class TitleScreen extends BasicScreen {
    constructor(titleText,width=canvas.width,height=canvas.height) {
        super()
        if (titleText instanceof createjs.DisplayObject) {
            this.title = titleText;
            this.addChild(this.title);
        } else if (titleText) {
            this.title = this.createText(titleText,"bold 60px regul,Arial,Microsoft Yahei");
        }
        this.title.x = width - this.title.getBounds().width >> 1;
        this.title.y = height / 3 + 20;
    }
...
class ScoreBoard extends BasicScreen {
    ...
    createTextElement(key, val, xpos = 0, ypos = 0, { titleImg, font ="30px regul,Arial,Microsoft Yahei", color ="#fff", borderFont, offX = 0, valueType = "span", width = 150, height = 50, max = 5 } = {}) {
...
class InstructionScreen extends BasicScreen {
    constructor(title, content, actions, width, height) {
        ...
        modal.setBounds(0,0,ele.clientWidth,ele.clientHeight)
    }
}
//--------------------------------------------------进度条----------------------------------------------------------------------
class LoaderBar extends BasicScreen {
    constructor(titleText = "loading...", width = 500, height = 30) {
        ...
        this.x=stage.width-b.width>>1;
        this.y=stage.height-b.height>>1;
    }
    createBar(width, height) {
        // let bar = this.createDom("meter");
...
class Fps extends BasicScreen {
    constructor(x = 10, y = 10) {
        super();
        this.fps = this.createText("", "30px regul,Arial,Microsoft Yahei", "#f00")
        this.x = x;
        this.y = y;
    }
    onAdded() {
