...
export class Tunnel extends Game {
    ...
    runGame() {
        ...
        if(player.hitBounds()) this.gameOver=true;
        //障碍物
        this.moveActors(this.moveChildren);
        obstacleTick++;
        if(obstacleTick>obstacleDelay){
            obstacleTick=0;
            let obstacle=Obstacle.getActor();
