...
class SpaceHero extends Game {
    ...
    newLevel() {
        ...
        this.player=Ship.getActor();
        this.player.init(stage.width/2-32,stage.height-64)
    ...
    runGame() {
        ...
        //加入敌机
        this.addEnemy();
        this.moveContainer(this.moveChildren);//测试取消
    }
    addEnemy() {
        if (enemyIdex-- <= 0) {
            timeToEnemy -= DIFFICULTY;//测试取消
            timeToEnemy = Math.max(timeToEnemy, 10);
            let enemy = Enemy.getActor();
            enemy.init()
            this.addToMove(enemy);
...
class Ship extends Actor {
    ...
    act() {
        ...
        if (this.image.currentAnimation == "heroIdle") {
            let actor = this.hitActors(game.moveChildren, null,game.container);
            if (actor) {
                if (actor.type == "enemy") {
                    actor.recycle();
                    let explode = Explode.getActor();
                    game.container.addChild(explode);
                    explode.x = actor.x;
                    explode.y = actor.y;
                ...
                this.hp--;
                if (this.hp <= 0) {
                    let ex = Explode.getActor();
                    game.container.addChild(ex)
                    ex.x = this.x;
                    ex.y = this.y;
                    game.lives--
...
class Enemy extends Actor {
    ...
    act() {
        ...
        super.act();
        let actor = this.hitActors(game.floorChildren, null,game.container);
        if (actor) {
...
class Explode extends Actor{
    ...
    init(){
        this.image.gotoAndPlay("explosion")
    }
}