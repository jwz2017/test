...
export class SuperClick extends Game {
    ...
    waitComplete() {
        ...
        this.on('mousedown', (e) => {
            if (e.target.type === Ball.BAD) {
                ...
                this.scoreboard.update(ACHIEVE, achieve);
                //加入分数文本
                let txt = Actor.getActor(createjs.Text);
                txt.text = addScore;
                txt.scale = 1;
    ...
    runGame() {
        this.creatElement();
        this.moveActors(this.floorChildren);
        this.checkGame();
    }
    creatElement() {
        if (this.floorChildren.length < maxOnScreen && numCreated < numCircles) {
            var circle;
            if (Math.random() * 100 < percentBadCircle) {
                circle = Actor.getActor(Ball);
                circle.init(Ball.BAD);
            } else {
                circle = Actor.getActor(Ball);
                circle.init(Ball.GOOD);
                numCreated++;
            }
            this.floorLayer.addChild(circle);
...
class Ball extends createjs.Shape {
    ...
    init(type){
        this.type=type;
        if(type=Ball.GOOD)this.radius=15;
        else this.radius=10;
        this.scale=1;
        this.alpha=1;
